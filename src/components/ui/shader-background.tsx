...
import { shouldDisableHeavyEffects, isMobile } from "@/lib/performance";

const ShaderBackground = () => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const [disableShader, setDisableShader] = useState(false);
  const [renderInterval, setRenderInterval] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    if (disableHeavyEffects) {
      ...
      // Используем статичный градиент вместо шейдера
      return;
    }
    ...
    // Для мобильных устройств снижаем частоту кадров
  ...
  // Статичный фон для слабых устройств
  if (disableShader) {
    return (
      <div 
  ...
  return <canvas ref={canvasRef} className="fixed inset-0 h-full w-full -z-10" />;
};
